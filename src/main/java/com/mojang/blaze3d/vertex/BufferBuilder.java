...
public class BufferBuilder extends DefaultedVertexConsumer implements BufferVertexConsumer
{
    ...
    {
        if (this.mode == VertexFormat.Mode.QUADS && (this.sortX != pSortX || this.sortY != pSortY || this.sortZ != pSortZ))
        {
            this.sortX = pSortX;
            this.sortY = pSortY;
            this.sortZ = pSortZ;
    ...
    public void endVertex()
    {
        ...
        else
        {
            ...
            if (this.mode == VertexFormat.Mode.LINES || this.mode == VertexFormat.Mode.LINE_STRIP)
            {
                ...
                for (int j = this.nextElementByte - i; j < this.nextElementByte; ++j)
                {
                    this.buffer.put(this.buffer.get(j));
                }
