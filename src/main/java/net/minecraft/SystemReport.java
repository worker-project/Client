...
public class SystemReport
{
    ...
    private void putVirtualMemory(VirtualMemory pMemory)
    {
        ...
        {
            return String.format("%.2f", (float)pMemory.getVirtualMax() / 1048576.0F);
        });
        this.setDetail("Virtual memory used (MB)", () ->
        {
            return String.format("%.2f", (float)pMemory.getVirtualInUse() / 1048576.0F);
        });
        this.setDetail("Swap memory total (MB)", () ->
        {
            return String.format("%.2f", (float)pMemory.getSwapTotal() / 1048576.0F);
        });
        this.setDetail("Swap memory used (MB)", () ->
        {
            return String.format("%.2f", (float)pMemory.getSwapUsed() / 1048576.0F);
        });
    }
