...
public class TemptingSensor extends Sensor<PathfinderMob>
{
    ...
    protected void doTick(ServerLevel pLevel, PathfinderMob pAttacker)
    {
        Brain<?> brain = pAttacker.getBrain();
        List<Player> list = pLevel.players().stream().filter(EntitySelector.NO_SPECTATORS).filter((p_148342_) ->
        {
            return TEMPT_TARGETING.test(pAttacker, p_148342_);
        }).filter((p_148335_) ->
        {
            return pAttacker.closerThan(p_148335_, 10.0D);
        }).filter(this::playerHoldingTemptation).sorted(Comparator.comparingDouble(pAttacker::distanceToSqr)).collect(Collectors.toList());
