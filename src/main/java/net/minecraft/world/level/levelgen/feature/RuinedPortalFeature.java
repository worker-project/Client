...
public class RuinedPortalFeature extends StructureFeature<RuinedPortalConfiguration>
{
    ...
    private static Optional<PieceGenerator<RuinedPortalConfiguration>> pieceGeneratorSupplier(PieceGeneratorSupplier.Context<RuinedPortalConfiguration> p_197153_)
    {
        ...
        Rotation rotation = Util.a(Rotation.values(), worldgenrandom);
    ...
    private static int findSuitableY(Random pRandom, ChunkGenerator pChunkGenerator, RuinedPortalPiece.VerticalPlacement pVerticalPlacement, boolean pAirPocket, int pBaseHeight, int pYSpan, BoundingBox pBoundingBox, LevelHeightAccessor pLevel)
    {
        ...
        {
            i = pBaseHeight - pYSpan + Mth.randomBetweenInclusive(pRandom, 2, 8);
        }
        else
        {
            i = pBaseHeight;
        ...
        List<BlockPos> list1 = ImmutableList.of(new BlockPos(pBoundingBox.minX(), 0, pBoundingBox.minZ()), new BlockPos(pBoundingBox.maxX(), 0, pBoundingBox.minZ()), new BlockPos(pBoundingBox.minX(), 0, pBoundingBox.maxZ()), new BlockPos(pBoundingBox.maxX(), 0, pBoundingBox.maxZ()));
        List<NoiseColumn> list = list1.stream().map((p_197138_) ->
        {
            return pChunkGenerator.getBaseColumn(p_197138_.getX(), p_197138_.getZ(), pLevel);
