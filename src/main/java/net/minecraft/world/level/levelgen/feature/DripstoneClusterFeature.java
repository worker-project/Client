        ...
        if (optional.isPresent())
        {
            ...
            if (optionalint.isPresent() || optionalint1.isPresent())
            {
                ...
                if (optionalint2.isPresent() && flag2 && !this.isLava(pLevel, pPos.atY(optionalint2.getAsInt())))
                {
                    ...
                    {
                        i3 = Math.max(0, j + Mth.randomBetweenInclusive(pRandom, -pDensity.maxStalagmiteStalactiteHeightDiff, pDensity.maxStalagmiteStalactiteHeightDiff));
                    }
                    else
                    {
                        i3 = this.getDripstoneHeight(pRandom, pX, pZ, pHeight, p_159601_, pDensity);
                    }
                }
                else
                {
