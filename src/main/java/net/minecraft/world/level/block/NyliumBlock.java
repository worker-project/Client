...
public class NyliumBlock extends Block implements BonemealableBlock
{
    ...
    public void performBonemeal(ServerLevel pLevel, Random pRand, BlockPos pPos, BlockState pState)
    {
        ...
        {
            NetherFeatures.CRIMSON_FOREST_VEGETATION_BONEMEAL.value().place(pLevel, chunkgenerator, pRand, blockpos);
        }
        else if (blockstate.is(Blocks.WARPED_NYLIUM))
        {
            NetherFeatures.WARPED_FOREST_VEGETATION_BONEMEAL.value().place(pLevel, chunkgenerator, pRand, blockpos);
            NetherFeatures.NETHER_SPROUTS_BONEMEAL.value().place(pLevel, chunkgenerator, pRand, blockpos);

            if (pRand.nextInt(8) == 0)
            {
                NetherFeatures.TWISTING_VINES_BONEMEAL.value().place(pLevel, chunkgenerator, pRand, blockpos);
            }
        }
    }
}
