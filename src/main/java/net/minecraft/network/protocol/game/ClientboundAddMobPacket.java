...
public class ClientboundAddMobPacket implements Packet<ClientGamePacketListener>
{
    ...
    public ClientboundAddMobPacket(LivingEntity pBuffer)
    {
        this.id = pBuffer.getId();
        this.uuid = pBuffer.getUUID();
        this.type = Registry.ENTITY_TYPE.getId(pBuffer.getType());
        this.x = pBuffer.getX();
        this.y = pBuffer.getY();
        this.z = pBuffer.getZ();
        this.yRot = (byte)((int)(pBuffer.getYRot() * 256.0F / 360.0F));
        this.xRot = (byte)((int)(pBuffer.getXRot() * 256.0F / 360.0F));
        this.yHeadRot = (byte)((int)(pBuffer.yHeadRot * 256.0F / 360.0F));
        double d0 = 3.9D;
        Vec3 vec3 = pBuffer.getDeltaMovement();
