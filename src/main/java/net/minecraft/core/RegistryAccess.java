...
default <E> Registry<E> ownedRegistryOrThrow(ResourceKey<? extends Registry<? extends E>> pRegistryKey)
    {
...
default Stream<RegistryAccess.RegistryEntry<?>> networkSafeRegistries()
    ...
    private static <E> void readRegistry(DynamicOps<JsonElement> pOps, RegistryLoader.Bound pDestinationRegistryAccess, RegistryAccess.RegistryData<E> pData)
    {
        DataResult <? extends Registry<E >> dataresult = pDestinationRegistryAccess.overrideRegistryFromResources(pData.key(), pData.codec(), pOps);
