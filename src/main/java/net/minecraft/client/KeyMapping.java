...
public class KeyMapping implements Comparable<KeyMapping>
{
    ...
    public static void setAll()
    {
        ...
        {
            if (keymapping.key.getType() == InputConstants.Type.KEYSYM && keymapping.key.getValue() != InputConstants.bv.getValue())
            {
    ...
    {
        return this.category.equals(p_90841_.category) ? I18n.a(this.name).compareTo(I18n.a(p_90841_.name)) : CATEGORY_SORT_ORDER.get(this.category).compareTo(CATEGORY_SORT_ORDER.get(p_90841_.category));
    }
    ...
    {
        return this.key.equals(InputConstants.bv);
    }

    public boolean matches(int pKeysym, int pScancode)
    {
        if (pKeysym == InputConstants.bv.getValue())
        {
    ...
    public static Map<String, KeyMapping> getALL() {
        return ALL;
    }
}
