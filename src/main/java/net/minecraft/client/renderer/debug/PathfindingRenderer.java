...
public class PathfindingRenderer implements DebugRenderer.SimpleDebugRenderer
{
    ...
    public static void renderPath(Path pPath, float pMaxDistanceToWaypoint, boolean pRenderOpenAndClosedSet, boolean pRenderText, double pX, double p_113626_, double pY)
    {
        ...
        doRenderPath(pPath, pMaxDistanceToWaypoint, pRenderOpenAndClosedSet, pRenderText, pX, p_113626_, pY);
    ...
    {
        renderPathLine(pPath, pX, p_113645_, pY);
        BlockPos blockpos = pPath.getTarget();

        if (distanceToCamera(blockpos, pX, p_113645_, pY) <= 80.0F)
        {
            DebugRenderer.renderFilledBox((new AABB((double)((float)blockpos.getX() + 0.25F), (double)((float)blockpos.getY() + 0.25F), (double)blockpos.getZ() + 0.25D, (double)((float)blockpos.getX() + 0.75F), (double)((float)blockpos.getY() + 0.75F), (double)((float)blockpos.getZ() + 0.75F))).move(-pX, -p_113645_, -pY), 0.0F, 1.0F, 0.0F, 0.5F);

            for (int i = 0; i < pPath.getNodeCount(); ++i)
            {
                Node node = pPath.getNode(i);

                if (distanceToCamera(node.asBlockPos(), pX, p_113645_, pY) <= 80.0F)
                {
                    float f = i == pPath.getNextNodeIndex() ? 1.0F : 0.0F;
                    float f1 = i == pPath.getNextNodeIndex() ? 0.0F : 1.0F;
                    DebugRenderer.renderFilledBox((new AABB((double)((float)node.x + 0.5F - pMaxDistanceToWaypoint), (double)((float)node.y + 0.01F * (float)i), (double)((float)node.z + 0.5F - pMaxDistanceToWaypoint), (double)((float)node.x + 0.5F + pMaxDistanceToWaypoint), (double)((float)node.y + 0.25F + 0.01F * (float)i), (double)((float)node.z + 0.5F + pMaxDistanceToWaypoint))).move(-pX, -p_113645_, -pY), f, 0.0F, f1, 0.5F);
                }
            }
        }
        ...
        if (pRenderOpenAndClosedSet)
        {
            ...
            {
                if (distanceToCamera(node2.asBlockPos(), pX, p_113645_, pY) <= 80.0F)
                {
                    DebugRenderer.renderFilledBox((new AABB((double)((float)node2.x + 0.5F - pMaxDistanceToWaypoint / 2.0F), (double)((float)node2.y + 0.01F), (double)((float)node2.z + 0.5F - pMaxDistanceToWaypoint / 2.0F), (double)((float)node2.x + 0.5F + pMaxDistanceToWaypoint / 2.0F), (double)node2.y + 0.1D, (double)((float)node2.z + 0.5F + pMaxDistanceToWaypoint / 2.0F))).move(-pX, -p_113645_, -pY), 1.0F, 0.8F, 0.8F, 0.5F);
                }
            }

            for (Node node3 : pPath.getOpenSet())
            {
                if (distanceToCamera(node3.asBlockPos(), pX, p_113645_, pY) <= 80.0F)
                {
                    DebugRenderer.renderFilledBox((new AABB((double)((float)node3.x + 0.5F - pMaxDistanceToWaypoint / 2.0F), (double)((float)node3.y + 0.01F), (double)((float)node3.z + 0.5F - pMaxDistanceToWaypoint / 2.0F), (double)((float)node3.x + 0.5F + pMaxDistanceToWaypoint / 2.0F), (double)node3.y + 0.1D, (double)((float)node3.z + 0.5F + pMaxDistanceToWaypoint / 2.0F))).move(-pX, -p_113645_, -pY), 0.8F, 1.0F, 1.0F, 0.5F);
                }
            }
        }
        ...
        if (pRenderText)
        {
            for (int j = 0; j < pPath.getNodeCount(); ++j)
            {
                ...
                if (distanceToCamera(node1.asBlockPos(), pX, p_113645_, pY) <= 80.0F)
                {
    ...
    public static void renderPathLine(Path pPath, double pX, double p_113618_, double pY)
    {
        ...
        for (int i = 0; i < pPath.getNodeCount(); ++i)
        {
            ...
            if (!(distanceToCamera(node.asBlockPos(), pX, p_113618_, pY) > 80.0F))
            {
                ...
                int l = j >> 8 & 255;
                int i1 = j & 255;
                bufferbuilder.vertex((double)node.x - pX + 0.5D, (double)node.y - p_113618_ + 0.5D, (double)node.z - pY + 0.5D).color(k, l, i1, 255).endVertex();
            }
        }
