...
public class TextureAtlasSprite implements AutoCloseable
{
    ...
    {
        this(pAtlas, pSpriteInfo, pMipLevel, pStorageX, pStorageY, pX, pY, pImage, (ShadersTextureType)null);
    }
    ...
    protected TextureAtlasSprite(TextureAtlas atlasTextureIn, TextureAtlasSprite.Info spriteInfoIn, int mipmapLevelsIn, int atlasWidthIn, int atlasHeightIn, int xIn, int yIn, NativeImage imageIn, ShadersTextureType spriteShadersTypeIn)
    {
        ...
        {
            if (!spriteInfoIn.name().getPath().endsWith("_leaves"))
            {
    ...
    void a(int p_118376_, int p_118377_, NativeImage[] p_118378_)
    {
        boolean flag = false;
    ...
    public VertexConsumer wrap(VertexConsumer pConsumer)
    {
        ...
        {
            MultiBufferSource.BufferSource multibuffersource$buffersource = pConsumer.getRenderTypeBuffer();
    ...
    public TextureAtlasSprite makeSpriteShaders(ShadersTextureType type, int colDef, TextureAtlasSprite.AnimatedTexture parentAnimatedTexture)
    {
        ...
        if (this.resourceManager.hasResource(resourcelocation1))
        {
            try
            {
                Resource resource = this.resourceManager.getResource(resourcelocation1);
        ...
        textureatlassprite.usesParentAnimationTime = matchesTiming(textureatlassprite.animatedTexture, parentAnimatedTexture);
        return textureatlassprite;
    }
    ...
    private void setTerrain(boolean terrainIn)
    {
        ...
        if (this.shaders && !this.isSpriteSingle)
        {
            ...
            {
                this.spriteNormal = this.makeSpriteShaders(ShadersTextureType.NORMAL, -8421377, this.animatedTexture);
            }

            if (this.spriteSpecular == null && Shaders.configSpecularMap)
            {
                this.spriteSpecular = this.makeSpriteShaders(ShadersTextureType.SPECULAR, 0, this.animatedTexture);
            }
        }
    }

    private static boolean matchesTiming(TextureAtlasSprite.AnimatedTexture at1, TextureAtlasSprite.AnimatedTexture at2)
    {
        if (at1 != null && at2 != null)
        {
            ...
            {
                boolean flag = at1.interpolationData != null;
                boolean flag1 = at2.interpolationData != null;
                ...
                {
                    List<TextureAtlasSprite.FrameInfo> list = at1.frames;
                    List<TextureAtlasSprite.FrameInfo> list1 = at2.frames;
                    ...
                    if (list != null && list1 != null)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                TextureAtlasSprite.FrameInfo textureatlassprite$frameinfo = list.get(i);
                                TextureAtlasSprite.FrameInfo textureatlassprite$frameinfo1 = list1.get(i);

                                if (textureatlassprite$frameinfo == null || textureatlassprite$frameinfo1 == null)
                                {
                                    return false;
                                }

                                if (textureatlassprite$frameinfo.index != textureatlassprite$frameinfo1.index)
                                {
                                    return false;
                                }

                                if (textureatlassprite$frameinfo.time != textureatlassprite$frameinfo1.time)
                                {
                                    return false;
                                }
                            }
    ...
    class AnimatedTexture implements Tickable, AutoCloseable
    {
        ...
        private void uploadFrame(int pFrameIndex)
        {
            ...
            TextureAtlasSprite.this.a(i, j, TextureAtlasSprite.this.mainImage);
        }
    ...
    final class InterpolationData implements AutoCloseable
    {
        ...
        void uploadInterpolatedFrame(TextureAtlasSprite.AnimatedTexture pTexture)
        {
            ...
            if (i != j)
            {
                ...
                TextureAtlasSprite.this.a(0, 0, this.activeFrame);
            }
        }
        ...
        private int mix(double pRatio, int p_118456_, int pFirst)
        {
            return (int)(pRatio * (double)p_118456_ + (1.0D - pRatio) * (double)pFirst);
        }
