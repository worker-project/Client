...
public class Gui extends GuiComponent
{
    ...
    public Gui(Minecraft pMinecraft)
    {
        ...
        this.chatListeners.get(ChatType.CHAT).add(new StandardChatListener(pMinecraft));
        this.chatListeners.get(ChatType.CHAT).add(chatlistener);
        this.chatListeners.get(ChatType.SYSTEM).add(new StandardChatListener(pMinecraft));
        this.chatListeners.get(ChatType.SYSTEM).add(chatlistener);
        this.chatListeners.get(ChatType.GAME_INFO).add(new OverlayChatListener(pMinecraft));
            ...
            while (true)
            {
                ...
                if (mobeffectinstance.showIcon())
                {
                    ...
                    list.add(() ->
                    {
                        ...
                        blit(pPoseStack, i1 + 3, j1 + 3, this.getBlitOffset(), 18, 18, textureatlassprite);
                    });

                    if (effectrenderer != null)
                    {
                        effectrenderer.renderHUDEffect(mobeffectinstance, this, pPoseStack, j, k, (float)this.getBlitOffset(), f);
                    }
                }
            }
        }
        ...
        if (this.toolHighlightTimer > 0 && !this.lastToolHighlight.isEmpty())
        {
            ...
            if (l > 0)
            {
                ...
                if (font != null)
                {
                    ...
                    font.draw(pPoseStack, component.getVisualOrderText(), (float)j, (float)k, 16777215 + (l << 24));
                }
                else
                {
                    this.getFont().drawShadow(pPoseStack, component, (float)j, (float)k, 16777215 + (l << 24));
                }
    ...
    public void handleChat(ChatType pChatType, Component pComponent, UUID pSenderId)
    {
        ...
        {
            for (ChatListener chatlistener : this.chatListeners.get(pChatType))
            {
