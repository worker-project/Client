    ...
    public void render(PoseStack pPoseStack)
    {
        ...
        if (this.isListening && !this.subtitles.isEmpty())
        {
            ...
            for (SubtitleOverlay.Subtitle subtitleoverlay$subtitle1 : this.subtitles)
            {
                ...
                pPoseStack.pushPose();
                pPoseStack.translate((double)((float)this.minecraft.getWindow().getGuiScaledWidth() - (float)l * 1.0F - 2.0F), (double)((float)(this.minecraft.getWindow().getGuiScaledHeight() - 30) - (float)(i * (i1 + 1)) * 1.0F), 0.0D);
