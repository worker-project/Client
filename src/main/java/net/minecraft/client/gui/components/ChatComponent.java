...
public class ChatComponent extends GuiComponent
{
        ...
        if (!this.isChatHidden())
        {
            ...
            if (k > 0)
            {
                ...
                for (int j1 = 0; j1 + this.chatScrollbarPos < this.trimmedMessages.size() && j1 < j; ++j1)
                {
                    ...
                    if (guimessage != null)
                    {
                        int k1 = pTickCount - guimessage.getAddedTime();
                        ...
                        if (k1 < 200 || flag)
                        {
                            ...
                            if (i2 > 3)
                            {
                                ...
                                {
                                    fill(pPoseStack, -4, (int)(d5 - d2), 0 + l + 4, (int)d5, j2 << 24);
                                }
                                ...
                                {
                                    this.minecraft.font.draw(pPoseStack, guimessage.getMessage(), 0.0F, (float)((int)(d5 + d3)), 16777215 + (i2 << 24));
                                }
                                else
                                {
                                    this.minecraft.font.drawShadow(pPoseStack, guimessage.getMessage(), 0.0F, (float)((int)(d5 + d3)), 16777215 + (i2 << 24));
                                }
                ...
                if (!this.chatQueue.isEmpty())
                {
                    ...
                    pPoseStack.pushPose();
                    pPoseStack.translate(0.0D, 0.0D, 50.0D);
                    fill(pPoseStack, -2, 0, l + 4, 9, j3 << 24);
                    RenderSystem.enableBlend();
                    pPoseStack.translate(0.0D, 0.0D, 50.0D);
                    this.minecraft.font.drawShadow(pPoseStack, new TranslatableComponent("chat.queue", this.chatQueue.size()), 0.0F, 1.0F, 16777215 + (l2 << 24));
                    pPoseStack.popPose();
                    RenderSystem.disableBlend();
                }
                ...
                if (flag)
                {
                    ...
                    if (k3 != l3)
                    {
                        ...
                        fill(pPoseStack, 0, -i4, 2, -i4 - l1, k4 + (j4 << 24));
                        fill(pPoseStack, 2, -i4, 1, -i4 - l1, 13421772 + (j4 << 24));
                    }
                }
    ...
    public void addMessage(Component pChatComponent, int pChatLineId)
    {
    ...
    private void addMessage(Component p_93791_, int p_93792_, int p_93793_, boolean p_93794_)
    {
        ...
        EventBus.INSTANCE.post(new ServerChatEvent(p_93791_, flag));
