...
public class RecipeButton extends AbstractWidget
{
    ...
    {
        ItemStack itemstack = this.getOrderedRecipes().get(this.currentIndex).getResultItem();
        pNarrationElementOutput.add(NarratedElementType.TITLE, new TranslatableComponent("narration.recipe", itemstack.getHoverName()));

        if (this.collection.getRecipes(this.book.isFiltering(this.menu)).size() > 1)
        {
            pNarrationElementOutput.a(NarratedElementType.USAGE, new TranslatableComponent("narration.button.usage.hovered"), new TranslatableComponent("narration.recipe.usage.more"));
        }
        else
        {
