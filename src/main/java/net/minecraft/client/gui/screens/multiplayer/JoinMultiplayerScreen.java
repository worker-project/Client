...
public class JoinMultiplayerScreen extends Screen {
    ...
    protected void init() {
        ...
        if (this.initedOnce) {
            ...
            if(!servers.hasIP(serverSelectionList.HYPIXEL.getServerData().ip)) {
                servers.add(serverSelectionList.HYPIXEL.getServerData());
                servers.save();
            }
    ...
    private void editServerCallback(boolean p_99717_) {
        ...
        if (p_99717_ && serverselectionlist$entry instanceof ServerSelectionList.OnlineServerEntry && !this.editingServer.ip.toLowerCase().contains(SERVER_NAME.toLowerCase())) {
            ServerData serverdata = ((ServerSelectionList.OnlineServerEntry) serverselectionlist$entry).getServerData();
    ...
    private void addServerCallback(boolean p_99722_) {
        if (p_99722_ && !this.editingServer.ip.toLowerCase().contains(SERVER_NAME.toLowerCase())) {
    ...
    protected void onSelectedChange() {
        ...
        if (serverselectionlist$entry != null && !(serverselectionlist$entry instanceof ServerSelectionList.LANHeader)) {
            ...
            if (serverselectionlist$entry instanceof ServerSelectionList.OnlineServerEntry && !((ServerSelectionList.OnlineServerEntry) serverselectionlist$entry).getServerData().ip.equals(this.serverSelectionList.HYPIXEL.getServerData().ip)) {
